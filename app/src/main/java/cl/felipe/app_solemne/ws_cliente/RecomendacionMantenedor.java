...
import cl.felipe.app_solemne.Recomendacion;

public class RecomendacionMantenedor extends IMantenedor<Recomendacion> {


    public ListView listView;

    public RecomendacionMantenedor(Context context, ListView listView) {
        super(context);
        this.listView = listView;
    }

    @Override
    public Recomendacion save(Recomendacion reclamo) {
        if (!activeInternetConnection()) {
            Toast t = Toast.makeText(context, "Error: Conexión a internet requerida", Toast.LENGTH_LONG);
            t.show();
            return null;
        }
        String url = apiUrl + "/recomendacion/crear";

        JSONObject body = new JSONObject();

        try {
            body.put("comment", reclamo.getComment());
            body.put("usuarioId", reclamo.getUserId());
            body.put("autorId", reclamo.getAutorId());
    ...
    @Override
    public List<Recomendacion> findAll(int id) {
        if (!activeInternetConnection()) {
            Toast t = Toast.makeText(context, "Error: Conexión a internet requerida", Toast.LENGTH_LONG);
            t.show();
            return null;
        }
        String url = apiUrl + "/recomendacion?autorId=" + id;
        JSONObject body = new JSONObject();
                            ...
                            try {
                                ...
                                for (int i = 0; i < response.length(); i++) {
                                    ...
                                    data[i] = "ID: " + o.get("id").toString() + " - Usuario: " + o.get("nombreUsuario").toString() + " " +
                                            " - Autor: " +
                                            o.get("nombreAutor").toString() + " - Comentario: " +o.get("comment").toString()  ;
                                }
    ...
    @Override
    public Recomendacion find(int id) {
        return null;
    }
    ...
    public void delete(int id) {
        ...
        String url = apiUrl + "/recomendacion/borrar";

        JSONObject body = new JSONObject();

        try {
            body.put("recomendacionId", id);
    ...
    @Override
    public Recomendacion edit(Recomendacion reclamo) {
        if (!activeInternetConnection()) {
            Toast t = Toast.makeText(context, "Error: Conexión a internet requerida", Toast.LENGTH_LONG);
            t.show();
            return null;
        ...
        String url = apiUrl + "/recomendacion/actualizar";

        JSONObject body = new JSONObject();

        try {
            body.put("recomendacionId", reclamo.getId());
            body.put("comment", reclamo.getComment());
